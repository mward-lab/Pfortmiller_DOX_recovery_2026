...
#p53 HM DE 144R
```{r p53 target genes logFC heatmap 144R, fig.height=10, fig.width=6}
...
yh2ax_all <- bind_rows(h2ax_87, h2ax_75) %>% 
  as.data.frame() %>% 
  dplyr::select(-(contains("2nd")))
...
# Step 1: Calculate proportion positive
yh2ax_all_df <- yh2ax_all_df %>%
  mutate(Proportion_Positive = Positive_ROIs / Total_ROIs,
         Condition = Name,                       # Full treatment name
         Timepoint = gsub("DOX|DMSO", "", Name), # Extract timepoint
         Treatment = Drug,                       # Drug name
         Individual = factor(Individual),
         Condition_Fill = Condition)             # For coloring by condition

# Step 2: Plot boxplots per magnification including all conditions
for (mag in magnifications) {
  ...
  p <- ggplot(df_mag, aes(x = Timepoint, y = Proportion_Positive, fill = Condition_Fill)) +
    geom_boxplot(outlier.shape = NA, alpha = 0.6, position = position_dodge(width = 0.8)) +
    geom_jitter(aes(color = Individual), width = 0.15, size = 2, show.legend = FALSE) +
...
```{r Set up Dataframe for Boxplots IF Intensity}

sorted_summary <- summary_all %>% 
  arrange(Magnification, Treatment, Timepoint, Individual, desc(Nuclei_Count)) %>% 
  group_by(Magnification, Treatment, Timepoint, Individual) %>% 
...
```{r Plot Intensity Boxplots yH2AX}

#read in .csv summary files
read.csv("data/new/IF_all_image_intensity_summary.csv")
read.csv("data/new/IF_treatment_intensity_summary.csv")

#prepare data for plotting by removing 2nd only samples
...
magnifications <- unique(plot_df$Magnification)

for (mag in magnifications) {
  df_mag <- plot_df %>% filter(Magnification == mag)
  
  p <- ggplot(df_mag, aes(x = Treatment, y = Mean_Intensity)) +
    geom_boxplot(aes(fill = Treatment), alpha = 0.6) +
    geom_point(aes(color = Individual), position = position_dodge(width = 0.8), size = 2) +
    facet_wrap(~Timepoint) +
    scale_color_manual(values = ind_col) +
    scale_fill_manual(values = tx_col) +
    labs(
      title = paste0("yH2AX Mean Intensity - ", mag),
      y = "Mean Intensity per Nucleus (pixels)",
      x = "Treatment",
      fill = "Treatment",
      colour = "Individual"
    ) +
    ...
    # Statistical comparison: DOX vs DMSO per timepoint
    ggsignif::geom_signif(
      comparisons = list(c("DMSO", "DOX")),
...
####try making a percent of maximum to normalize into a percentage
max_intensity <- max(plot_df$Mean_Intensity, na.rm = TRUE)

plot_df_int <- plot_df %>%
  mutate(Percent_of_Max = (Mean_Intensity / max_intensity) * 100,
         Individual = factor(Individual, levels = c("Ind2","Ind4")),
         Treatment = factor(Treatment, levels = c("DOX","DMSO")),
         Timepoint = factor(Timepoint, levels = c("24T","24R","144R")))

magnifications <- unique(plot_df_int$Magnification)
ind_col <- c("Ind2" = "#45AE91", "Ind4" = "#8B3E9B")
tx_col <- c("DOX" = "#499FBD", "DMSO" = "#BBBBBC")

for (mag in magnifications) {
  df_mag <- plot_df_int %>% filter(Magnification == mag)
  
  p <- ggplot(df_mag, aes(x = Treatment, y = Percent_of_Max)) +
    geom_boxplot(aes(fill = Treatment), alpha = 0.6) +
    geom_point(aes(color = Individual), position = position_dodge(width = 0.8), size = 2) +
    facet_wrap(~Timepoint) +
    scale_color_manual(values = ind_col) +
    scale_fill_manual(values = tx_col) +
    scale_y_continuous(limits = c(0,110)) +  # allow slight buffer over 100%
    labs(
      title = paste0("yH2AX Mean Intensity (% of Max) - ", mag),
      y = "Percent of Maximum Intensity",
      x = "Treatment",
      fill = "Treatment",
      colour = "Individual"
    ) +
...
####now try median
median_df <- image_summary %>%
  filter(!is.na(Treatment)) 

#keep 5 most consistent images per condition and individual (since some have many more nuclei)
median_df_consistent <- median_df %>%
  group_by(Individual, Treatment, Timepoint, Magnification) %>%
  arrange(desc(Nuclei_Count)) %>%  # largest number of nuclei first
  slice_head(n = 5) %>%            # keep top 5
  ungroup()

#compute the percent of max median intensity
max_median <- max(median_df_consistent$Median_Intensity, na.rm = TRUE)

median_df_consistent <- median_df_consistent %>%
  mutate(Percent_of_Max = (Median_Intensity / max_median) * 100,
         Individual = factor(Individual, levels = c("Ind2","Ind4")),
         Treatment = factor(Treatment, levels = c("DOX","DMSO")),
         Timepoint = factor(Timepoint, levels = c("24T","24R","144R")))

magnifications <- unique(median_df_consistent$Magnification)
ind_col <- c("Ind2" = "#45AE91", "Ind4" = "#8B3E9B")
tx_col <- c("DOX" = "#499FBD", "DMSO" = "#BBBBBC")
...
#plot as above
for (mag in magnifications) {
  df_mag <- median_df_consistent %>% filter(Magnification == mag)
  ...
  p <- ggplot(df_mag, aes(x = Treatment, y = Percent_of_Max)) +
    ...
    scale_color_manual(values = ind_col) +
    scale_fill_manual(values = tx_col) +
    scale_y_continuous(limits = c(0,110)) +  # slight buffer over 100%
    labs(
      title = paste0("yH2AX Median Intensity (% of Max) - ", mag),
      y = "Percent of Maximum Median Intensity",
      x = "Treatment",
      fill = "Treatment",
      colour = "Individual"
    ) +
...
```{r Plot Intensity as Total Fluorescence mean x area}
