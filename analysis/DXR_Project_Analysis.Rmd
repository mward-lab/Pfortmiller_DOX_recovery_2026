...
forest_TOP2i <- ggplot(forest_summary, 
                       aes(y = Treatment, x = OR, 
                           ...
                           color = Group)) + 
  geom_point(size = 4) +
  geom_errorbarh(height = 0.5, size = 0.5, color = "black") +
...
studies_knowles <- list(
  AllGenes_Knowles = all_genes_knowles,
...
study_levels <- c("AllGenes_Knowles", "Marginal eQTLs", "Response eQTLs")
category_levels <- c("Sustained", "Recovered")
...
ggplot(df_motif_knowles, aes(x = Study, y = Proportion, fill = Category)) +
  ...
  geom_text(data = df_motif_knowles %>% group_by(Study) %>% summarise(TotalMotifGenes = unique(TotalMotifGenes)),
            aes(x = Study, y = 1, label = paste0("n = ", TotalMotifGenes)),
