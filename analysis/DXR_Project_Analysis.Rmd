...
timepoints <- c("24", "24R", "144R")

comparison_order <- c("24 vs 24R", "24R vs 144R", "24 vs 144R")
...
scatterplot_fig2d <- wrap_plots(plot_list, ncol = length(plot_list))

scatterplot_fig2d

#save plot to a pdf and to a png
ggsave("Fig2D_Scatterplot_fixedlims_EMP_250919.pdf", scatterplot_fig2d, width = 8, height = 6, device = cairo_pdf, path = "C:/Users/emmap/OneDrive/Desktop/DXR Manuscript Materials/Fig pdfs")
ggsave("Fig2D_Scatterplot_fixedlims_EMP_250919.png", scatterplot_fig2d, width = 12, height = 4, dpi = 300, type = "cairo", path = "C:/Users/emmap/OneDrive/Desktop/DXR Manuscript Materials/Fig pdfs")
...
# --- Step 1: Collect gene sets from all sources ---
all_genes <- unique(c(final_genes_1_RUV, final_genes_2_RUV))

# Add Knowles Supp4 + Supp5 gene sets
knowles4_ids <- unique(knowles_eqtls_supp4_annot$HGNC[!is.na(knowles_eqtls_supp4_annot$HGNC)])
knowles5_ids <- unique(knowles_eqtls_supp5_annot$HGNC[!is.na(knowles_eqtls_supp5_annot$HGNC)])
...
# --- Step 5: Factor Study and Category for proper order ---
study_levels <- c("AllGenes", "Fonoudi", "Liu", "Sapp", "Marginal eQTLs", "Response eQTLs")
category_levels <- c("Sustained", "Recovered")
...
# --- Step 6: Plot ---
ggplot(summary_motif, aes(x = Study, y = Proportion, fill = Category)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = scales::percent(Proportion, accuracy = 1)),
            position = position_stack(vjust = 0.5),
            size = 3.5, color = "white") +
  ...
  scale_x_discrete(labels = c("All", "Fonoudi", "Liu", "Sapp", "Marginal eQTLs", "Response eQTLs")) +
  theme_minimal(base_size = 14) +
  labs(
    y = "Proportion of Genes",
    x = "",
...
all_DEGs_dxr <- lapply(combined_toptables_dxr, function(df) {
  df %>% 
    dplyr::filter(adj.P.Val < 0.05) %>% 
    mutate(absFC = abs(logFC))
})
...
categorize_sustained <- function(gene, all_DEGs_dxr) {
  #pull tibbles
  t24T  <- all_DEGs_dxr[["24T"]]
  t144R <- all_DEGs_dxr[["144R"]]
  
  #pull absLogFC values if gene exists, else NA
  absFC_24T  <- if (gene %in% t24T$Entrez_ID)  t24T %>% filter(Entrez_ID == gene) %>% pull(absFC) else NA_real_
  absFC_144R <- if (gene %in% t144R$Entrez_ID) t144R %>% filter(Entrez_ID == gene) %>% pull(absFC) else NA_real_
  
  #assign categories based on comparison first
  if (!is.na(absFC_24T) & !is.na(absFC_144R)) {
    if (absFC_144R > absFC_24T) {
      return("absFC t144 > t0")
...
sustained_cats <- sapply(final_genes_2_RUV, categorize_sustained, all_DEGs_dxr = all_DEGs_dxr)
...
t24T <- combined_toptables_dxr[["24T"]] %>%
  dplyr::select(Entrez_ID, logFC, adj.P.Val) %>%
  mutate(
    logFC_24T = logFC,
    adjP_24T  = adj.P.Val,
    Sig_24T   = ifelse(adj.P.Val < 0.05, "yes", "no"),
    absFC_24T = abs(logFC)
  ) %>%
  dplyr::select(Entrez_ID, logFC_24T, adjP_24T, Sig_24T, absFC_24T)

t24R <- combined_toptables_dxr[["24R"]] %>%
  dplyr::select(Entrez_ID, logFC, adj.P.Val) %>%
  mutate(
    logFC_24R = logFC,
    adjP_24R  = adj.P.Val,
    Sig_24R   = ifelse(adj.P.Val < 0.05, "yes", "no"),
    absFC_24R = abs(logFC)
  ) %>%
  dplyr::select(Entrez_ID, logFC_24R, adjP_24R, Sig_24R, absFC_24R)

t144R <- combined_toptables_dxr[["144R"]] %>%
  dplyr::select(Entrez_ID, logFC, adj.P.Val) %>%
  mutate(
    logFC_144R = logFC,
    adjP_144R  = adj.P.Val,
    Sig_144R   = ifelse(adj.P.Val < 0.05, "yes", "no"),
    absFC_144R = abs(logFC)
  ) %>%
  dplyr::select(Entrez_ID, logFC_144R, adjP_144R, Sig_144R, absFC_144R)

#change type to match
df_sustained$Entrez_ID <- as.integer(df_sustained$Entrez_ID)
t24T$Entrez_ID <- as.integer(t24T$Entrez_ID)
t24R$Entrez_ID <- as.integer(t24R$Entrez_ID)
t144R$Entrez_ID <- as.integer(t144R$Entrez_ID)


# Join all timepoints to sustained genes
df_sustained_full <- df_sustained %>%
  left_join(t24T,   by = "Entrez_ID") %>%
  left_join(t24R,   by = "Entrez_ID") %>%
  left_join(t144R,  by = "Entrez_ID")
...
categorize_sustained_4 <- function(gene, all_DEGs_dxr) {
  #pull tibbles
  t24T  <- all_DEGs_dxr[["24T"]]
  t144R <- all_DEGs_dxr[["144R"]]
...
#this would be the 136 genes where t144 > t0
...
inc_sus_gene_list <- inc_sus_genes$Entrez_ID

#background
# all_genes

length(inc_sus_gene_list)
#136 genes in this set
#now 296 t144 > t0 when not filt degs

incsus_GOKEGG_RUV <- gost(query = inc_sus_gene_list,
                          organism = "hsapiens",
                          ordered_query = FALSE,
                          measure_underrepresentation = FALSE,
                          evcodes = FALSE,
...
incsus_GOKEGG_genes_RUV <- gostplot(incsus_GOKEGG_RUV, capped = FALSE, interactive = TRUE)
incsus_GOKEGG_genes_RUV

table_incsus_GOKEGG_genes_RUV <- incsus_GOKEGG_RUV$result %>% 
  dplyr::select(c(source, term_id, term_name, intersection_size, term_size, p_value))

table_incsus_GOKEGG_genes_RUV %>% 
  mutate_at(.vars = 6, .funs = scales::label_scientific(digits=4)) %>% 
  kableExtra::kable(.,) %>% 
  kableExtra::kable_paper("striped", full_width = FALSE) %>% 
  kableExtra::kable_styling(full_width = FALSE, position = "left", bootstrap_options = c("striped", "hover")) %>% 
...
#GO:BP
table_incsus_genes_GOBP_RUV <- table_incsus_GOKEGG_genes_RUV %>% 
  dplyr::filter(source=="GO:BP") %>% 
  dplyr::select(p_value, term_name, intersection_size) %>% 
  dplyr::slice_min(., n=10, order_by=p_value) %>% 
  mutate(log_val = -log10(p_value))

incsus_table_genes_GOBP_plot <- ggplot(table_incsus_genes_GOBP_RUV, aes(x = log_val, y = reorder(term_name, log_val), col= log_val)) +
  geom_point(aes(size = intersection_size)) +
...
print(incsus_table_genes_GOBP_plot)

save_plot(
  plot = incsus_table_genes_GOBP_plot,
  filename = "Sustained_t144vt0_GOBP_logval_all_EMP",
  folder = output_folder
)

#KEGG
table_incsus_genes_KEGG_RUV <- table_incsus_GOKEGG_genes_RUV %>% 
  dplyr::filter(source=="KEGG") %>% 
  dplyr::select(p_value, term_name, intersection_size) %>% 
  dplyr::slice_min(., n=10, order_by=p_value) %>% 
  mutate(log_val = -log10(p_value))

incsus_table_genes_KEGG_plot <- ggplot(table_incsus_genes_KEGG_RUV, aes(x = log_val, y = reorder(term_name, log_val), col= log_val)) +
  geom_point(aes(size = intersection_size)) +
...
print(incsus_table_genes_KEGG_plot)

save_plot(
  plot = incsus_table_genes_KEGG_plot,
  filename = "Sustained_t144vt0_KEGG_logval_all_EMP",
  folder = output_folder
)

```

##Venn Diagram for Figure 6D
```{r Venn Diagram }
