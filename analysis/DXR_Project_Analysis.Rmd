...
cor_matrix_spmn_k1_cpm_new <- cor(cpm_k1_rename,
                           y = NULL,
                           use = "everything",
                           method = "spearman")
...
save_complex_heatmap(
  ...
  filename = "SuppFig3_SpearmanCorHM_RUV_log2cpm_EMP_251022.pdf",
  folder = output_folder,
  width = 8,
  height = 6
)
...
venn_test_RUV <- list(DEG1_RUV, DEG2_RUV, DEG3_RUV)
ggVennDiagram(
  venn_test_RUV,
  category.names = c("DOX_24T", "DOX_24R", "DOX_144R")
...
all_genes_dxr <- lapply(combined_toptables_dxr, function(df) {
  df %>% 
    mutate(absFC = abs(logFC))
})
...
t24 <- combined_toptables_dxr[["t24"]] %>%
  dplyr::select(Entrez_ID, logFC, adj.P.Val) %>%
  mutate(
    logFC_t24 = logFC,
    adjP_t24  = adj.P.Val,
...
categorize_sustained_fixed <- function(gene, all_DEGs_dxr) {
...
sus_check <- df_sustained_full_all %>%
  mutate(
    Category_check = case_when(
      absFC_t0 < absFC_t144 ~ "proSus",  #inc over time
      absFC_t0 > absFC_t144 ~ "recSus",  #dec over time
      absFC_t0 == absFC_t144 ~ "equal",
      TRUE ~ "missing"
    )
  )
...
#I pulled data from the GWAS catalog for HF from the HERMES study 2020


hf_gwas_data <- read_delim("D:/DXR Project/HF_GWAS/"C:\Users\emmap\RDirectory\Recovery_RNAseq\Recovery_5FU\data\new\HF_GWAS\pub\databases\gwas\summary_statistics\GCST009001-GCST010000\GCST009541\ShahS_31919418_HeartFailure.gz"", 
    delim = "\t", escape_double = FALSE, 
    trim_ws = TRUE)
View(elife_33480_supp4_v2)
...
```{r GO KEGG proSus New Set1}

proSus_gene_list <- proSus_df$Entrez_ID
length(proSus_gene_list) #264 genes after changing the definition
...
print(proSus_table_genes_KEGG_plot)
#there are 0 KEGG terms associated with these genes
...
#GO KEGG Set2 proSus
```{r GO KEGG terms proSus Set 2 163 genes}
proSus2_gene_list <- proSus_df2$Entrez_ID
length(proSus2_gene_list) #163 genes after changing the definition

#background
all_genes

proSus2_GOKEGG_RUV <- gost(query = proSus2_gene_list,
                          organism = "hsapiens",
                          ordered_query = FALSE,
                          measure_underrepresentation = FALSE,
                          evcodes = FALSE,
...
proSus2_GOKEGG_genes_RUV <- gostplot(proSus2_GOKEGG_RUV, capped = FALSE, interactive = TRUE)
proSus2_GOKEGG_genes_RUV

table_proSus2_GOKEGG_genes_RUV <- proSus2_GOKEGG_RUV$result %>% 
  dplyr::select(c(source, term_id, term_name, intersection_size, term_size, p_value))

table_proSus2_GOKEGG_genes_RUV %>% 
  mutate_at(.vars = 6, .funs = scales::label_scientific(digits=4)) %>% 
  kableExtra::kable(.,) %>% 
  kableExtra::kable_paper("striped", full_width = FALSE) %>% 
  kableExtra::kable_styling(full_width = FALSE, position = "left", bootstrap_options = c("striped", "hover")) %>% 
...
#GO:BP
table_proSus2_genes_GOBP_RUV <- table_proSus2_GOKEGG_genes_RUV %>% 
  dplyr::filter(source=="GO:BP") %>% 
  dplyr::select(p_value, term_name, intersection_size) %>% 
  dplyr::slice_min(., n=10, order_by=p_value) %>% 
  mutate(log_val = -log10(p_value))

proSus2_table_genes_GOBP_plot <- ggplot(table_proSus2_genes_GOBP_RUV, aes(x = log_val, y = reorder(term_name, p_value))) +
  geom_point(aes(size = intersection_size)) +
  ggtitle("proSus Set2 Enriched GO:BP Terms")+
  xlab(expression("-log"[10]~"p-value"))+
  guides(size= guide_legend(title = "# of \n intersected \n terms"))+
  ylab("GO:BP term")+
  scale_y_discrete(labels = scales::label_wrap(30))+
...
print(proSus2_table_genes_GOBP_plot)

# save_plot(
#   plot = proSus2_table_genes_GOBP_plot,
#   filename = "Sustained_proSus_Set2_GOBP_logval_all_EMP",
#   folder = output_folder
# )

#KEGG
table_proSus2_genes_KEGG_RUV <- table_proSus2_GOKEGG_genes_RUV %>% 
  dplyr::filter(source=="KEGG") %>% 
  dplyr::select(p_value, term_name, intersection_size) %>% 
  dplyr::slice_min(., n=10, order_by=p_value) %>% 
  mutate(log_val = -log10(p_value))

proSus2_table_genes_KEGG_plot <- ggplot(table_proSus2_genes_KEGG_RUV, aes(x = log_val, y = reorder(term_name, p_value))) +
  geom_point(aes(size = intersection_size)) +
  ggtitle("proSus Set2 Enriched KEGG Terms")+
  xlab(expression("-log"[10]~"p-value"))+
  guides(size= guide_legend(title = "# of \n intersected \n terms"))+
  ylab("KEGG term")+
  scale_y_discrete(labels = scales::label_wrap(30))+
...
print(proSus2_table_genes_KEGG_plot)
#there are 0 KEGG terms associated with these genes

# save_plot(
#   plot = proSus2_table_genes_KEGG_plot,
#   filename = "Sustained_proSus_set2_KEGG_logval_all_EMP",
#   folder = output_folder
# )
...
proSus_fin_summary <- df_sustained_proSus %>%
  group_by(Study, Category) %>%
  summarise(Count = n(), .groups = "drop") %>%
  mutate(Proportion = Count / sum(Count),
         Percent = round(Proportion * 100, 1))
...
#read in my sustained all df (501 genes)
...
proSus_gene_list <- readRDS("data/new/RUV/CMF/proSus_gene_list.RDS")
notproSus_gene_list <- readRDS("data/new/RUV/CMF/notproSus_gene_list.RDS")
...
#Pull out genes from GO/KEGG analysis in certain categories
```{r GO KEGG example genes from terms}
#I have several terms that I am interested in exploring further, since I have enrichment of DNA damage genes in proSus and notproSus but no p53 enrichment, that is all in notproSus


```


```{r logFC line plots proSus vs notproSus}
...
filt_toptable_proSus_RUV <- combined_toptables_dxr_RUV %>%
...
plot_proSus_logFC <- ggplot() +
  ...
  labs(title = "logFC — proSus Genes (n = 163)",
       y = "logFC", x = "Time") +
  theme_custom() +
  theme(legend.position = "right")
...
save_plot(plot_proSus_logFC, 
          filename = "proSus_lineplot_logFC_EMP_251112",
          folder = output_folder)

#now do the same for the notproSus genes
#create a toptable with all of the genes for proSus
filt_toptable_notproSus_RUV <- combined_toptables_dxr_RUV %>%
...
plot_notproSus_logFC

save_plot(plot_notproSus_logFC, 
          filename = "notproSus_lineplot_logFC_EMP_251112",
          folder = output_folder)
...
#   filename = "Lineplot_proSus_logFC_EMP_251112",
#   folder = output_folder
# )
# save_plot(
