...
Metadata <- readRDS("data/Metadata_update.RDS")
...
length(recSus_overlap) #37 genes overlapping
recSus_overlap
...
deg_wide <- combined_toptables_dxr_RUV %>%
  ...
  dplyr::select(Entrez_ID, SYMBOL, Timepoint, adj.P.Val) %>%
  pivot_wider(
...
categorize_sustained <- function(gene, all_DEGs_dxr) {
  ...
  absFC_t0  <- if (gene %in% t0$Entrez_ID)  t0 %>% filter(Entrez_ID == gene) %>% pull(absFC) else NA_real_
  absFC_t144 <- if (gene %in% t144$Entrez_ID) t144 %>% filter(Entrez_ID == gene) %>% pull(absFC) else NA_real_
...
df_sustainedt144$Entrez_ID <- as.integer(df_sustained$Entrez_ID)
...
#this would be the 205 genes where t144 > t0
...
incsus_table_genes_GOBP_plot <- ggplot(table_incsus_genes_GOBP_RUV, aes(x = log_val, y = reorder(term_name, p_value))) +
  geom_point(aes(size = intersection_size)) +
...
save_plot(
  ...
  filename = "Sustained_t144_more_t0_GOBP_logval_all_EMP",
  folder = output_folder
)
...
incsus_table_genes_KEGG_plot <- ggplot(table_incsus_genes_KEGG_RUV, aes(x = log_val, y = reorder(term_name, p_value))) +
  geom_point(aes(size = intersection_size)) +
...
save_plot(
  ...
  filename = "Sustained_t144_more_t0_KEGG_logval_all_EMP",
  folder = output_folder
)

```

```{r Go Analysis t144 < t0 genes}
#I changed this one from filtering for sig DEGs to all
# df_sustained_full_all <- df_sustained_full
# saveRDS(df_sustained_full_all, "data/new/RUV/CMF/sustained_proportions_dataframe.RDS")
...
dec_sus_gene_list <- dec_sus_genes$Entrez_ID

#background
# all_genes

length(dec_sus_gene_list)
...
```{r Proportional Venn Inc Dec Sus}
...
```{r log2cpm examples of overlap genes}

#proSus genes examples with overlap from CT set
proSus_CT_overlap <- intersect(inc_sus_gene_list, cardiotox_set)
proSus_CT_overlap
length(proSus_CT_overlap)  #should be 1
...
#now check to see if these genes overlap properly to other sets too
SusOver_test_recSus <- recSus_genes[final_genes_2_RUV %in% recSus_genes] 
SusOver_test_proSus <- proSus_genes[final_genes_2_RUV %in% proSus_genes] 

#sanity check against Recovered motif group
recSus_genes[recSus_genes %in% final_genes_1_RUV]
proSus_genes
