...
row.names(motif.fit_RUV_logNC_cpm$bestmotif$clustlike) <- row.names(lognormcounts_CMF_cpm)
cmf_logNC_cpm <- lognormcounts_CMF_cpm$bestmotif$clustlike %>% 
  as.data.frame() %>% 
...
Sets_cmf_cpmNC <- H3K27me3_toptable_list %>% 
  mutate(cluster=case_when(genes %in% final_genes_1_RUV$. ~ "Set1",
                           genes %in% final_genes_2_RUV$. ~ "Set2",
                           TRUE ~ "not_assigned")) %>% 
...
study_levels <- c("Liu", "Sapp", "DIC", "AllGenes")
...
ggplot(summary_table, aes(x = Study, y = Proportion, fill = Category)) +
  ...
  scale_x_discrete(labels = c("Liu", "Sapp", "Fonoudi", "All Genes")) +
  theme_minimal(base_size = 14) +
  labs(
...
# --- Step 3: Build dataset for all studies ---
studies <- list(
  Liu   = liu_ids,
  Sapp  = sapp_ids,
  Fonoudi   = dic_set,
...
study_levels <- c("Liu", "Sapp", "Fonoudi", "AllGenes")
category_levels <- c("Recovered", "Sustained", "Unassigned")
