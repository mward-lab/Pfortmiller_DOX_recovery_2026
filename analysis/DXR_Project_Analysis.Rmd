...
```{r IF macros for quantification - Fiji}

```

##Read in ROI summary sheets from above macros in Fiji
...
#collapse per individual for plotting (pre-normalized)

individual_summary_step4 <- consistent_images_top_nuclei %>%
...
```{r Normalize Boxplots by Max Per Mag}
#calculate the maximum percentage of red intensity per each magnification
max_percent_per_mag <- individual_summary_step4 %>%
  group_by(Magnification) %>%
  summarise(Max_Percent = max(Mean_Percent_Positive, na.rm = TRUE), .groups = "drop")

#then make a summary with the percentages for normalization
individual_summary_step5 <- individual_summary_step4 %>%
  left_join(max_percent_per_mag, by = "Magnification") %>%
  mutate(Norm_Percent_Positive = (Mean_Percent_Positive / Max_Percent) * 100)
...
stats_t_table_IF <- consistent_images_top_nuclei %>%
  filter(Treatment %in% c("DOX","DMSO")) %>%
  group_by(Magnification, Timepoint) %>%
  summarise(
    t_test = list(t.test(Mean_Percent_Red_Positive[Treatment=="DOX"],
                         Mean_Percent_Red_Positive[Treatment=="DMSO"])),
    .groups = "drop"
  ) %>%
  rowwise() %>%
  mutate(
  ...
  select(Magnification, Timepoint, t_stat, df, p_value, signif)

print(stats_t_table_IF)
...
plot_step2 <- ggplot(consistent_images_top_nuclei, 
                     aes(x = Condition, y = Mean_Percent_Red_Positive)) +
  geom_boxplot(alpha = 0.6, fill = "lightgray", outlier.shape = NA) +
  geom_point(aes(color = Individual), alpha = 0.6, size = 2, position = position_identity()) +
  facet_wrap(~Magnification, scales = "free_x") +
...
#step 3: mean by individual of 5 images

individual_summary_step3 <- consistent_images %>%
  group_by(Individual, Magnification, Condition) %>%
  summarise(Mean_Percent_Positive = mean(Mean_Percent_Red_Positive, na.rm = TRUE),
            .groups = "drop")

plot_step3 <- ggplot(individual_summary_step3, 
                     aes(x = Condition, y = Mean_Percent_Positive)) +
  geom_boxplot(alpha = 0.6, fill = "lightgray", outlier.shape = NA) +
  geom_point(aes(color = Individual), alpha = 0.6, size = 3, position = position_identity()) +
  facet_wrap(~Magnification, scales = "free_x") +
  labs(title = "Step 3: Mean Percent Positive per Individual",
...
#step 4: normalized by individual for percent red positive by area

plot_step4 <- ggplot(individual_summary_step5, 
                     aes(x = Condition, y = Norm_Percent_Positive)) +
  geom_boxplot(alpha = 0.6, fill = "lightgray", outlier.shape = NA) +
  geom_point(aes(color = Individual), alpha = 0.6, size = 3, position = position_identity()) +
  facet_wrap(~Magnification, scales = "free_x") +
  labs(title = "Step 4: Normalized Percent Red Positive (0–100 scale)",
       x = "Condition", y = "Normalized Percent Positive") +
  theme_bw() +
  scale_x_discrete(limits = condition_levels) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
...
#my thoughts as to why 10X is so low in Ind4: lower image intensity and harder to see overall
...
step5_boxplot_10x <- plot_individuals_boxplot_step5(individual_summary_step5, "10X", stats_t_table_IF)
step5_boxplot_20x <- plot_individuals_boxplot_step5(individual_summary_step5, "20X", stats_t_table_IF)
...
overlap_labels <- plot_df %>%
  filter(Category == "Overlap") %>%
  left_join(perm_summary_df %>% select(Treatment, Significant), by = "Treatment") %>%
  mutate(
