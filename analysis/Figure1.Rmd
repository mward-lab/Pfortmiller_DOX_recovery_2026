...
```{r pdf saving function, echo=TRUE, message=FALSE, warning=FALSE}
...
#this is actually ind 2 according to my RNAseq - 78-1 F
...
#this is actually ind 3 according to my RNAseq - 75-1 F
...
Figure 1D - yH2AX Quantification
#Fiji/ImageJ Macros
...
#read in the data from the macros that you ran for each individual
#I am going to focus on 20X for these now, so remove any non-20X samples in the next step
#also ensure that you label each with their respective individual number for ease later on

####Ind 1 - 84-1####
# ind1_roi <- read.csv("D:/DXR Project/IF yH2AX/if yh2ax dxr 84-1/outline new/analyzed/All_ROIs_Combined_84-1_EMP_250916.csv") %>% 
#   mutate(Individual = "Ind1") %>% 
#   mutate(Magnification = "20X") #specify magnification since all were 20X so not in labels

# saveRDS(ind1_roi, "data/new/IF_quant/All_ROIs_Combined_84-1_Ind1.RDS")

####Ind 2 - 87-1####
# ind2_roi <- read.csv("D:/DXR Project/IF yH2AX/if yh2ax dxr 87-1/outline new/analyzed/All_ROIs_Combined_87-1_EMP_250825.csv") %>% 
#   mutate(Individual = "Ind2",
...
# saveRDS(ind2_roi, "data/new/IF_quant/All_ROIs_Combined_87-1_Ind2.RDS")

####Ind 4 - 75-1####
# ind4_roi <- read.csv("D:/DXR Project/IF yH2AX/if yh2ax dxr 75-1/outline new/analyzed/All_ROIs_Combined_75-1_EMP_250825.csv") %>%
...
# saveRDS(ind4_roi, "data/new/IF_quant/All_ROIs_Combined_75-1_Ind4.RDS")

#keep all of this for posterity and read them in for plots

ind1_roi <- readRDS("data/new/IF_quant/All_ROIs_Combined_84-1_Ind1.RDS")

ind2_roi <- readRDS("data/new/IF_quant/All_ROIs_Combined_87-1_Ind2.RDS")

ind4_roi <- readRDS("data/new/IF_quant/All_ROIs_Combined_75-1_Ind4.RDS")
...
# ind_roi_all <- bind_rows(ind1_roi, ind2_roi, ind4_roi) %>%
#   mutate(
#     Treatment = case_when(
...
Boxplot Step 1 ROI Level
```{r Step 1 Boxplot ROI Level All Individuals}
...
ind_cols_if <- c("Ind1" = "#FFB482",
              "Ind2" = "#8AC6D1",
              "Ind4" = "#F9E07F")

condition_levels <- c("DOX_24T", "VEH_24T", "DOX_24R", "VEH_24R", "DOX_144R", "VEH_144R")
...
plot_step1 <- ind_roi_all %>% 
  ...
  labs(title = "Step 1: ROI Level % Red Positive",
       x = "Condition", y = "Percent Red Positive Pixels") +
...
Collapse to Image Level Summaries
```{r Image Level Summary Sheet}
...
ind2_roi <- ind2_roi %>% mutate(Percent_Red_Positive_Area = (Red_Positive_Pixels / ROI_Area) * 100)
...
#  mutate(Individual = "Ind1",          Magnification = "20X")

 # ind2_img_summary <- collapse_to_image(ind2_roi) %>%
 #  mutate(Individual = "Ind2",
 #         Magnification = case_when(
...
# roi_image_level_summary <-
#   bind_rows(ind1_img_summary, ind2_img_summary, ind4_img_summary)
...
#    dplyr::filter(Magnification == "20X")
# 
# saveRDS(roi_image_level_summary,
#         "data/new/IF_quant/roi_image_level_summary_all.RDS")

```

Read in Dataframe
```{r ROI Summary Dataframe}
#now that I've created the image level summary table for all individuals above, read it in so I don't have to generate it multiple times

roi_image_level_summary <- readRDS("data/new/IF_quant/roi_image_level_summary_all.RDS")

# write.csv(roi_image_level_summary, "data/new/IF_quant/roi_image_level_summary_all.csv")

```

Make a Metadata Table
```{r Metadata from image level summaries}
...
# roi_folders <- list(
#   Ind1 = "D:/DXR Project/IF yH2AX/if yh2ax dxr 84-1/DAPI counts new",
#   Ind2 = "D:/DXR Project/IF yH2AX/if yh2ax dxr 87-1/DAPI counts new",
#   Ind4 = "D:/DXR Project/IF yH2AX/if yh2ax dxr 75-1/DAPI counts new"
# )
...
ind2_metadata <- read_csvs(roi_folders$Ind2, "Ind2") %>% 
...
image_level_metadata <- bind_rows(ind1_metadata, ind2_metadata, ind4_metadata)
...
image_level_metadata <- image_level_metadata %>%
  ...
  dplyr::select(Individual, Treatment, Timepoint, Image, Nuclei_Count) %>% 
  mutate(
    Condition = paste0(Treatment, "_", Timepoint),
    Magnification = "20X"
  )
...
Combine metadata with summary sheet
```{r Combine summary sheet with metadata sheet}
...
merged_image_summary <-
  ...
  dplyr::select(Individual, Treatment, Timepoint, Condition, Magnification, Image, Nuclei_Count) %>%
  dplyr::left_join(roi_image_level_summary, by = c("Image", "Individual", "Magnification"))

merged_image_summary <- merged_image_summary %>% mutate(Condition = factor(Condition, levels = c(condition_levels)))
...
Step 2 Boxplots Image Level Summary
```{r Boxplots Collapsed by Image}
...
plot_step2 <- ggplot(merged_image_summary, 
  ...
  labs(title = "Step 2: Image-Level Mean Red Positive Pixels",
       x = "Condition", y = "Mean Red Positive Pixels") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = "top") +
...
Step 3 Select Consistent Images Per Individual
```{r Images by Consistent Nuclei Count}
...
# Define the good images for Ind1 (IDs = the 4-digit part before "_DAPI")
images_Ind1 <- tibble::tribble(
  ~Condition,   ~ID,
  "DOX_24T",  "0012",
  "DOX_24T",  "0013",
  "DOX_24T",  "0017",
...
ind1_filtered <- merged_image_summary %>%
...
merged_image_summary_filt <- merged_image_summary %>% 
  dplyr::filter(Individual != "Ind1") %>% 
  bind_rows(ind1_filtered)

#now that I've put Ind1 back in, let's select images based on nuclei for Ind2 and Ind4
...
ind2_filtered <- select_top3_images(merged_image_summary, "Ind2")
ind4_filtered <- select_top3_images(merged_image_summary, "Ind4")

#filter Ind 1 to get the same scores at the end
ind1_filtered <- ind1_filtered %>%
    group_by(Condition) %>%
    mutate(
        med_nuclei = median(Nuclei_Count, na.rm = TRUE),
...
merged_image_summary_final <- bind_rows(
  merged_image_summary %>% filter(!Individual %in% c("Ind1", "Ind2", "Ind4")),
  ind1_filtered,
  ind2_filtered,
  ind4_filtered
) %>% 
...
plot_step3 <- ggplot(merged_image_summary_final, 
                     aes(x = Condition, y = Mean_Norm_Percent_Red_Positive)) +
  geom_boxplot(outlier.shape = NA) +
  geom_point(aes(color = Individual), size = 2, alpha = 1,
             position = position_identity()) +
  scale_x_discrete(limits = condition_levels) +
  labs(title = "Step 3: Select 3 Images Per Individual for Consistent Nuclei",
...
ttests_step3 <- merged_image_summary_final %>%
  ...
  add_significance("p") %>%
  mutate(
    y.position = max(plot_data$Mean_Norm_Percent_Red_Positive, na.rm = TRUE) * 1.05
  )
...
plot_step3_test <- ggplot(merged_image_summary_final, 
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = "top") +
  stat_pvalue_manual(sig_table_step3, label = "signif", tip.length = 0)
...
Step 4: Normalize DOX/VEH before Individual Level Collapse
```{r Step 4 Normalization}
...
plot_step4 <- ggplot(merged_image_summary_norm, 
                        aes(x = Condition, y = Norm_Percent_Red_Positive)) +
  geom_boxplot(alpha = 0.6, outlier.shape = NA, fill = "lightgray") +
  geom_point(aes(color = Individual), size = 2, alpha = 0.7, position = position_identity()) +
  scale_x_discrete(limits = condition_levels) +
...
Step 5 Collapse by Individual
```{r Step 5 Collapse by Individual Points}
...
plot_step5 <- ggplot(individual_summary_step5, 
  ...
  geom_boxplot() +
  geom_point(aes(color = Individual), size = 3, alpha = 1, position = position_identity()) +
  scale_y_continuous(limits = c(0, 100)) +
  scale_x_discrete(limits = condition_levels) +
  scale_color_manual(values = ind_cols_if) +
  stat_pvalue_manual(ttest_table_step5, label = "p.signif", 
                     xmin = "group1", xmax = "group2", tip.length = 0.02) +
